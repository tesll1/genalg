...
class PolynomialInput(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.line_edit = QLineEdit()    # поле для ввода
        
        self.line_edit.setPlaceholderText("Например: x^5 - 3*x^2 + 4")  # текст-подсказка
        self.line_edit.setText("x^5 - 3*x^2 + 4")                       # значение по умолчанию
        self.line_edit.returnPressed.connect(self.on_enter_pressed)     # подключение сигнала
        layout.addWidget(self.line_edit)                                # добавление поля ввода
    ...
    def set_polynomial(self, text: str):
        """ Устанавливает текст полинома программно """
...
class IntervalInput(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout = QHBoxLayout()              # горизонтальное расположение
        self.setLayout(layout)
        
        layout.addWidget(QLabel("["))       # левая граница
        self.l_spin = QDoubleSpinBox()      # поле для ввода левой границы
        self.l_spin.setRange(-1000, 1000)   # минимальное и максимальное значение
        self.l_spin.setValue(-10)           # значение по умолчанию
        
        self.l_spin.valueChanged.connect(self.emit_value) # подключение сигнала
        layout.addWidget(self.l_spin)
        layout.addWidget(QLabel(", "))

        self.r_spin = QDoubleSpinBox()      # правая граница
    ...
    def set_interval(self, l: float, r: float):
        """ Устанавливает интервал программно """
        self.l_spin.setValue(l)
        self.r_spin.setValue(r)
    
    def emit_value(self):
        """ Отправляет сигнал с текущими значениями """
